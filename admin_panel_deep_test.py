...
class AdminPanelTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if bookings:
                    ...
                    print(f"      Deposit: {sample.get('deposit_received', False)}")
                
                return True
            else:
                print(f"   ❌ Weekly bookings failed: {response.status_code} - {response.text}")
                return False
    ...
    def test_monthly_bookings(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/booking/admin/monthly?year={year}&month={month}",
                headers={"Authorization": f"Bearer {self.token}"}
            )
            ...
            if response.status_code == 200:
                ...
                print(f"   ❌ Monthly bookings failed: {response.status_code} - {response.text}")
                return False
    ...
    def test_cancel_booking(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   ❌ Booking cancellation failed: {response.status_code} - {response.text}")
                return False
    ...
    def test_change_password(self):
        ...
        # Test with invalid current password to avoid actually changing password
        try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if waitlist:
                    ...
                    print(f"      ID: {sample.get('id')}")
                    print(f"      Name: {sample.get('name')}")
                    print(f"      Preferred Date: {sample.get('preferred_date')}")
                    print(f"      Preferred Time: {sample.get('preferred_time')}")
                
                return True
            else:
                print(f"   ❌ Waitlist retrieval failed: {response.status_code} - {response.text}")
                return False
